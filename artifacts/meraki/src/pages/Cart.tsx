...
export default function Cart() {
  ...
  const handleWhatsAppCheckout = () => {
    ...
    let message = "Olá! Gostaria de finalizar meu pedido na Meraki ✨\n\n";
    message += "Produtos:\n";
    cart.forEach(item => {
      message += `• ${item.productName}\n`;
      message += `  Tamanho: ${item.size} | Qtd: ${item.quantity} | ${formatCurrency(item.price * item.quantity)}\n`;
    });
  ...
  if (cart.length === 0) {
    return (
      ...
      >
        <h1 className="font-serif italic font-light mb-6" style={{ fontSize: "2.4rem", color: BEGE }}>
          Sua sacola está vazia
        </h1>
        <p className="font-sans font-light text-sm mb-10" style={{ color: GOLD, letterSpacing: "0.06em" }}>
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16 max-w-4xl">
      <h1 className="font-serif italic font-light mb-12" style={{ fontSize: "2.4rem", color: BEGE }}>
        Sua Sacola
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-6">
          {cart.map(item => (
            ...
            >
              <div className="shrink-0" style={{ width: "6rem", aspectRatio: "1/1", backgroundColor: CARD_BG }}>
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex justify-between items-start gap-2">
                  <div>
                    ...
                    <p className="font-sans font-light text-xs mt-1" style={{ color: GOLD, letterSpacing: "0.1em" }}>
                      Tamanho: {item.size}
                    </p>
                  </div>
                  <button
                ...
                <div className="mt-auto flex justify-between items-end pt-3">
                  <div className="flex items-center" style={{ border: `1px solid rgba(201,185,154,0.3)` }}>
                    <button
                    ...
                    <span className="w-8 text-center font-sans font-light text-sm" style={{ color: BEGE }}>
                      {item.quantity}
                    </span>
                    <button
                  ...
                  <div className="text-right">
                    <p className="font-sans font-light text-sm" style={{ color: GOLD }}>
                      {formatCurrency(item.price * item.quantity)}
                    </p>
                    {item.originalPrice && (
                      <>
                        <p className="font-sans font-light text-xs line-through" style={{ color: "rgba(245,240,235,0.35)" }}>
                          {formatCurrency(item.originalPrice * item.quantity)}
                        </p>
                        <p className="font-sans font-light text-[10px] mt-0.5" style={{ color: GOLD, letterSpacing: "0.05em" }}>
        ...
        <div className="lg:col-span-1">
          ...
          >
            <h2 className="font-serif italic font-light mb-6" style={{ fontSize: "1.5rem", color: BEGE }}>
              Resumo
            </h2>

            <div className="space-y-3 text-sm mb-6 pb-6" style={{ borderBottom: `1px solid rgba(201,185,154,0.15)` }}>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-sans font-light" style={{ fontSize: "0.6rem", letterSpacing: "0.15em", color: GOLD, textTransform: "uppercase" }}>
                  A calcular
                </span>
              </div>
            </div>
            ...
            <div className="flex justify-between items-end mb-8">
              ...
              <span className="font-sans font-light text-xl" style={{ color: GOLD }}>{formatCurrency(total)}</span>
            </div>
            ...
            <p
              ...
              style={{ fontSize: "0.6rem", color: GOLD, opacity: 0.6, letterSpacing: "0.08em" }}
            >
              Você será redirecionado ao WhatsApp com seu pedido preenchido.
            </p>
          </div>
        </div>
      </div>
